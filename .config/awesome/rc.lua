...
-- change notifications icon size
...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey },            "l",     function ()
            awful.spawn.with_shell("betterlockscreen -l") end,
              {description = "lock screen", group = "client"}),

    -- Spotify broken shortcuts
    awful.key({ },              "F8",   function()
            awful.spawn.with_shell("dbus-send --print-reply --dest=org.mpris.MediaPlayer2.spotify /org/mpris/MediaPlayer2 org.mpris.MediaPlayer2.Player.PlayPause") end,
              {description = "pause/play", group = "spotify"}),
    ...
    awful.key({ "Control" },              "Print",   function()
            awful.spawn.with_shell("maim -s | xclip -selection clipboard -t image/png") end,
              {description = "area screenshot", group = "maim"}),

    awful.key({ },              "Print",   function()
            awful.spawn.with_shell("maim -i $(xdotool getactivewindow) | xclip -selection clipboard -t image/png") end,
              {description = "window screenshot", group = "maim"}),

    -- Powermenu
    awful.key({ "Control", "Mod1" },            "Delete",     function ()
            awful.spawn.with_shell(home_path .. "/.config/rofi/powermenu.sh") end,
...
-- Enable windows transparency and shadows
awful.spawn.with_shell("picom --fade-in-step 1 --fade-out-step 1 --inactive-opacity 1 -b")

-- Enable switch keyboard layouts with Win+Space and Alt+Shift
awful.spawn.with_shell("setxkbmap -layout 'us,ru' -option 'grp:win_space_toggle,grp:alt_shift_toggle'")

-- Enable touchpad one tap
